...
def _redirect_output(file_path=None):
...
class ChatAnalyzer:
    ...
    def analyze_chat(self, start_filter=None, end_filter=None, output_file=None):
    ...
    def analyze_vocabulary(self, start_filter=None, end_filter=None, output_file=None):
        with _redirect_output(output_file):
    ...
    def check_occurrence(self, required_groups, excluded_terms=None, start_filter=None, end_filter=None,
                         output_occurrence=False, output_file=None):
        with _redirect_output(output_file):
            data = self._filter(start_filter, end_filter)
            if not data:
...
# chat.analyze_chat(#Possible start and end date#, #Possible output file path#)
# chat.analyze_vocabulary(#Possible start and end date#, #Possible output file path#)
# chat.analyze_linguistic_style(#Possible start and end date#, #Possible output file path#)
# chat.analyze_emojis(#Possible start and end date#, #Possible output file path#)

# chat.check_occurrence( #Simple search Terms in double square brackets# [["Hey", "Hi", "Hello"]], #Possible start and end date#, #Possible output file path#)
...
#                       #Start date of analysis# start_filter=datetime(2024, 12, 21), #End date of analysis# end_filter=datetime(2025, 11, 5), #Save output in output.txt# output_file="output.txt")

# chat.analyze_chat(#Start date of analysis# start_filter=datetime(2023, 4, 23), #End date of analysis# end_filter=datetime(2025, 1, 1), #Save analysis output in output.txt# output_file="chat_analysis.txt")

if __name__ == '__main__':
    chat = ChatAnalyzer('input/chat.txt')

    chat.analyze_chat()
    #chat.analyze_vocabulary(output_file="output_chat_analyzation.txt")
